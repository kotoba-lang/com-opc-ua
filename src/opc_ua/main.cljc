(ns opc_ua.main
  "Kotodama WASM entrypoint for the opc_ua clean-room actor (L4) — Clojure port.
  ...
  facts (`opc_ua.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "opc_ua")
(def tier "L4")
...
(def entity-specs
  [{:entity "Node"   :plural "nodes"   :id-prefix "opcua_nod"
    :fields [:nodeId :nodeClass :browseName :displayName :description :writeMask :accessLevel :valueRank :historizing]
    :required [:nodeClass :browseName]
    :coerce {:nodeClass :int :writeMask :int :accessLevel :int :valueRank :int :historizing :bool}
    :refs {}}
   {:entity "ReferenceDescription"   :plural "referencedescriptions"   :id-prefix "opcua_ref"
    :fields [:referenceTypeId :isForward :nodeId :browseName :displayName :nodeClass :typeDefinition]
    :required [:isForward :browseName]
    :coerce {:isForward :bool :nodeClass :int}
    :refs {:nodeId "Node"}}
   {:entity "ReadValueId"   :plural "readvalueids"   :id-prefix "opcua_rea"
    :fields [:nodeId :attributeId :indexRange :dataEncoding]
    :required [:indexRange :dataEncoding]
    :coerce {:attributeId :int}
    :refs {:nodeId "Node"}}
   {:entity "MonitoredItem"   :plural "monitoreditems"   :id-prefix "opcua_mon"
    :fields [:clientHandle :monitoringMode :samplingInterval :queueSize :discardOldest]
    :required [:clientHandle :monitoringMode]
    :coerce {:clientHandle :int :monitoringMode :int :samplingInterval :float :queueSize :int :discardOldest :bool}
    :refs {}}
   {:entity "CreateSubscriptionRequest"   :plural "createsubscriptionrequests"   :id-prefix "opcua_cre"
    :fields [:requestedPublishingInterval :requestedLifetimeCount :requestedMaxKeepAliveCount :maxNotificationsPerPublish :publishingEnabled :priority]
    :required [:requestedPublishingInterval :requestedLifetimeCount]
    :coerce {:requestedPublishingInterval :float :requestedLifetimeCount :int :requestedMaxKeepAliveCount :int :maxNotificationsPerPublish :int :publishingEnabled :bool :priority :int}
    :refs {}}
   {:entity "ReadRequest"   :plural "readrequests"   :id-prefix "opcua_rea"
    :fields [:maxAge :timestampsToReturn]
    :required [:maxAge :timestampsToReturn]
    :coerce {:maxAge :float :timestampsToReturn :int}
    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"opc_ua.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "opc_ua-compat" :tier tier :entities entities} 200])
...
(defn start! [] :opc_ua-compat/ready)
